    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea
            className="w-full px-6 py-6 text-slate-900 bg-white border border-slate-300 rounded-xl placeholder-slate-400 outline-none focus:outline-none focus:border-slate-400 transition resize-none h-40"
        ...
        <div className="space-y-6 pt-12 border-t border-slate-100">
            ...
            {generations.map((gen)=>(
            <div key={gen._id} className="group bg-white rounded-2x1 border border-slate-100 p-5 hover:border-red-200 transition-all relative overflow-hidden">
            <div className="flex flex-col h-full space-y-4">

            <div className="flex items-center justify-between">
            <span className="text-xs text-slate-400 uppercase tracking-widest">
            {new Date(gen.createdAt).toLocaleString()}</span>
            ...
            <p className="text-sm text-slate-600 line-clamp-3 leading-relaxed flex-1">{gen.content}</p>
                ...
                bg-slate-50">
                <img src={gen.mediaUrl} alt="Gen" className="w-full aspect-video
                object-cover opacity-90 group-hover: opacity-100 transition-opacity"/>
                </div>
                )}
            ...
            <p className="text-slate-400 text-sm">No content generated yet. Try
            generating some content using the AI.</p>
            </div>
            )
            }
        ...
        <div className="bg-white rounded-xl shadow-2x1 w-full max-w-2xl border
        ...
        <button onClick={()=>setActiveScheduler (null)} className="p-2 rounded-full
        ...
        <XIcon className="size-5"/>
          ...
          <div className="bg-slate-50 rounded-2x1 p-6 border border-slate-100 space-y-4">
          <p className="text-slate-800 text-sm leading-relaxed whitespace-pre-wrap">
          {activeScheduler.prompt}</p>
          </div>


          <div className="bg-slate-50 rounded-2x1 p-6 border border-slate-100 space-y-4">
          <p className="text-slate-800 text-sm leading-relaxed whitespace-pre-wrap">
          {activeScheduler.content}</p>
          {activeScheduler.mediaUrl && <img src={activeScheduler.mediaUrl}
          alt="preview" className="w-full aspect-video object-cover rounded-xl border
